...
CREATE TABLE places (
  ...
  paused        TINYINT(1)   NOT NULL DEFAULT 0,   -- тимчасово зачинений
...
- **`confirmed_by`** зберігає різницю між «Google підтвердив» і «модератор
  ...
  нього два джерела.
- **`paused`** — заклад тимчасово зачинений: ремонт, сезон, війна. З карти він
  не зникає (це не те саме, що «закрито назавжди»), але людина має про це знати,
  інакше приїде й упреться в двері. У бандл їде саме це булеве поле, а не статус
  Google: значення `CLOSED_TEMPORARILY` лишається в `google_links` і назовні не
  виходить. 29.08.2026 таких 153 з 6503.
...
виявились одним закладом.

Випадків два, і головний — не той, що здається. Рідший: людина додала місце, а
через півроку воно приїхало з OSM під своїм id. Частіший: **той самий заклад
заведений в OSM двічі** — нодою й контуром будівлі або двома нодами за кілька
метрів. На карті це дві мітки, і обидві з галочкою. 29.08.2026 таких пар
зведено 64.
...
CREATE TABLE place_uid_aliases (
  ...
  merged_by  BIGINT UNSIGNED NULL,   -- хто злив; NULL для пайплайна
...
**Псевдонім — це ще й злиття даних.** Запис, що зникає, віддає тому, що
лишається, поля, яких у того немає: телефон, сайт, години, фото. Мапер, який
завів другу точку, часто вписав саме те, чого бракує першій. Своє лишається
своїм — координати, назва, вид, звʼязка з Google і дати правок описують саме
той обʼєкт, який лишився, і брати їх від сусіда не можна.

**Хто з двох лишається — вирок людини, а не автомата.** Пайплайн пропонує
того, у кого є адреса й більше заповнених полів (за рівності — ноду, а не
полігон: мапер ставить ноду на вхід, а від контуру будівлі лишається центр
прямокутника). Пропозицію видно на сторінці разом із обома записами, і людина
може обрати інший бік — на дані це не впливає, бо вони переїдуть у будь-якому
разі; впливає на те, яка точка правильно стоїть на карті.

`source = 'auto'` ставиться, коли збіглась і картка Google, і адреса: там
вирішувати нема чого. Решта — `manual`.
...
CREATE TABLE google_links (
  ...
  score      DECIMAL(3,2) NULL,       -- схожість назв на момент збігу
  source     ENUM('auto','text','replay','manual') NOT NULL DEFAULT 'auto',
  missed     SMALLINT NOT NULL DEFAULT 0,
  checked_at DATE NOT NULL,           -- коли востаннє ШУКАЛИ (раз на рік)
  status_at  DATE NULL,               -- коли востаннє питали СТАТУС (раз на пів року)
  PRIMARY KEY (osm_id),
  UNIQUE KEY uq_gid (gid),
  KEY ix_checked (checked_at),
  KEY ix_status (status_at)
) ENGINE=InnoDB;
```

**`UNIQUE KEY uq_gid` — не оптимізація, а правило.** Одна картка Google — один
наш заклад. Збіг рахується для кожної точки окремо, і ніщо не заважає двом
нашим закладам обрати одного кандидата: так буває, коли Google не знає одного
з них, і найкращим для нього стає найближчий сусід. 29.08.2026 таких спорів
знайшлось 79 — серед них «Terra», що вчепилась у картку «Klukva & Brukva» з
нульовою схожістю назв, самою лише відстанню у 8 метрів. Спір виграє краща
схожість, за рівної — менша відстань; ручна привʼязка не програє нікому.

**Дві дати, а не одна.** Пошук закладу й перевірка його статусу — різні задачі
з різною ціною: пошук іде по клітинках сітки (один запит на двадцять закладів),
перевірка — по одному запиту на заклад. Статус міняється частіше, ніж
зʼявляються нові заклади, тому `status_at` крутиться вдвічі швидше.

**`score` потрібен не для звіту.** Ним розсуджується спір за картку, і без нього
лишається тільки відстань — поганий суддя: сусід за три метри в центрі це
звичайна справа, а от назва випадково не збігається.
...
CREATE TABLE place_removals (
  uid        VARCHAR(20) NOT NULL,
  source     ENUM('manual','google','osm','rule','dupe') NOT NULL,
...
| `rule` | пайплайн, за назвою з `not_ours_rules` | пайплайн, коли правило прибрати | так, повністю |
| `dupe` | пайплайн, зайвий запис зведеної пари | пайплайн, коли зняти псевдонім | так, повністю |

Три нижні рядки — **виведені**: пайплайн робить їх наново щопрогону, і зникла
причина знімає надгробок сама. Ручний живе окремо й не переглядається ніколи.

`rule` — це мережі, яких на карті не буває: пивні прилавки навинос, що в OSM
стоять як `amenity=pub`. Поштучні надгробки тут не рятують — мережа відкриває
нову точку, і вона приходить наступним прогоном уже з іншим osm id.

`dupe` — зайвий запис пари, зведеної через `place_uid_aliases`. Надгробок тут
потрібен, щоб заклад не потрапив у бандл двічі; сама ж пара живе в псевдонімах,
і саме туди йдуть позначки людей.
...
CREATE TABLE place_edits (
  uid        VARCHAR(20) NOT NULL,
  field      ENUM('lat','lng','address','kind','name','hours') NOT NULL,
...
перекривати `lat`, `lng`, `address` і `kind` (константа `EDITABLE` у
`fetch-places.js`).

**`kind` відкрито 29.08.2026.** Вид закладу приходить із тегів OSM і більше
нізвідки, а маппер помиляється: «Гавань» у Дніпрі стояла як `pub`, хоч це кафе.
Помилка не косметична — від виду залежить іконка на мітці й фільтр. Звірка з
незалежним джерелом дала 1419 закладів із другою думкою, з них 371 розбіжність.
...
Два рядки про одне місце: доданий людиною заклад приїхав із OSM або той самий
заклад заведений в OSM двічі.

Ловиться це **спільним `place_id`** у межах 150 метрів, а не схожістю назв.
Схожість тут поганий суддя в обидва боки: «Aroma Kava» за 80 метрів — це дві
точки мережі, а «Три бобра» і «Три бобри - студія кави» — один заклад. Спільна
картка Google не залишає місця для тлумачень.

Далі вирішує адреса:

| адреси | що робить пайплайн |
|---|---|
| збігаються | зводить сам, `place_uid_aliases` із `source = 'auto'` |
| різні або відсутні | сюди, на вирок людини |

Адреси порівнюються без пробілів, дефісів і регістру: «Слобожанський проспект
1-П» і «1п» — та сама адреса, записана двома маперами.
...
CREATE TABLE merge_candidates (
  ...
  keep_uid  VARCHAR(20) NULL,        -- який із двох лишити; NULL = пропозиція не змінена
  verdict   ENUM('same','different') NULL,
...
Вердикт `same` створює рядок у `place_uid_aliases`, а `keep_uid` каже, у який
бік. За 29.08.2026: 31 пара на вирок, з них 17 виявились одним закладом.

**Далі 150 метрів пари не буває.** Спільний `place_id` за пів кілометра — це не
дубль, а помилкова привʼязка, і лікується вона в `google_links`, не тут.

---

## 8. База: службові таблиці

### `not_ours_rules` — назви, яких на карті не буває

```sql
CREATE TABLE not_ours_rules (
  id       BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  needle   VARCHAR(120) NOT NULL,   -- підрядок у назві, регістр не важить
  why      VARCHAR(255) NULL,
  added_by BIGINT UNSIGNED NULL,
  added_at DATETIME NOT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY uq_needle (needle)
) ENGINE=InnoDB;
```

Мережі, що торгують пивом навинос, стоять в OSM як `amenity=pub`, хоч це
прилавок без залу. Поштучні надгробки їх не беруть: мережа відкриває нову точку,
і вона приходить наступним прогоном уже з іншим osm id — а отже й з іншим `uid`.

Збіг — підрядком, без огляду на регістр і на те, яким із пʼяти символів
написаний апостроф: «Bear's Beer» ловить і «Bear’s Beer», і «Bear's Beer №3».
Кожен збіг дає надгробок із `source = 'rule'`, і виводиться він щопрогону
наново: прибрали правило — заклади повернулись самі.

Список короткий і росте рідко, тому окремого інтерфейсу не треба: правило додає
модератор, а пайплайн забирає його разом із рештою накладки.
...
```json
{
  ...
  "googleLinks": { "osm:n10003860121": { "gid": "ChIJ…", "status": "OPERATIONAL", "score": 1, "checkedAt": "2026-08-28", "statusAt": "2026-08-29" } }
}
```
...
`curated/twins.json`, `curated/dupe-suspects.json`, `curated/not-ours.json`.
...
1. **Пайплайн ніколи не пише в накладку і в контур людей.** Виняток єдиний:
   рядки `place_removals` із `source` у `('google','osm','rule','dupe')`.
...
10. **Одна картка Google — один наш заклад.** `UNIQUE KEY uq_gid` у
    `google_links`: два заклади з одним `place_id` носять чужий статус і зникають
    удвох, коли та картка закриється.
11. **Прогін зі старою накладкою не запускається** — скрипт падає.
